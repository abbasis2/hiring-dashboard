    ...
    <div className="space-y-3">
      ...
      <div className="card-shell overflow-hidden p-0">
        <div className="overflow-x-auto">
          <table className="min-w-[1650px] divide-y divide-[var(--border)] text-sm">
            ...
            <tbody className="divide-y divide-[var(--border)]">
              {rows.map((role) => (
                <tr key={role.id} className="align-top transition-all duration-200 hover:bg-[var(--bg-elevated)]">
                  ...
                  <td className="px-3 py-3"><select className="w-36 rounded-lg border border-[var(--border)] bg-[var(--bg-elevated)] px-2 py-2" onChange={(event) => updateDraft(role, { hired_gender: event.target.value })} value={role.hired_gender}><option key="__blank_gender" value="">Not specified</option>{options.gender.map((option) => <option key={option} value={option}>{option}</option>)}</select></td>
                  <td className="px-3 py-3"><input className="w-36 rounded-lg border border-[var(--border)] bg-[var(--bg-elevated)] px-2 py-2" onChange={(event) => updateDraft(role, { departure_event_date: event.target.value })} type="date" value={role.departure_event_date ?? ""} /></td>
                  ...
                  <td className="px-3 py-3"><textarea className="min-h-20 w-64 rounded-lg border border-[var(--border)] bg-[var(--bg-elevated)] px-2 py-2" onChange={(event) => updateDraft(role, { reason_why_next_steps: event.target.value })} value={role.reason_why_next_steps ?? ""} /></td>
